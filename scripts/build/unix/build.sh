...
echo "executing:::>${BUILD_SCP_COMMAND} -r $BUILD_RCM_SOURCE_PATH $BUILD_USER@${BUILD_HOST}:deploy <:::"
...
then
    ${BUILD_SCP_COMMAND} -r $BUILD_RCM_EXTERNAL_PATH/$BUILD_EXT_PATH $BUILD_USER@${BUILD_HOST}:deploy/RCM/rcm/client/external/turbovnc 
fi
...
artifact_platform_folder=$(${BUILD_SSH_COMMAND} "cd deploy/dist; ls")
artifact_folder=$(${BUILD_SSH_COMMAND} "cd deploy/dist/${artifact_platform_folder}; ls")
