...
############# setup #################################
ssh $BUILD_USER@${BUILD_HOST} ${BUILD_HOST_PORT} ${BUILD_SUDO_SETUP}

##############   inspect VM environment ##############
version=$(ssh $BUILD_USER@${BUILD_HOST} ${BUILD_HOST_PORT} "python --version"  2>&1)
...
version=$(ssh $BUILD_USER@${BUILD_HOST} ${BUILD_HOST_PORT} "${BUILD_VIRTUALENV_COMMAND} --version")
echo "virtualenv version $version"

ssh $BUILD_USER@${BUILD_HOST} ${BUILD_HOST_PORT} "if [ ! -f py3env/bin/activate ]; then ${BUILD_VIRTUALENV_COMMAND} py3env; fi"

##############   inspect virtualenv environment ##############
version=$(ssh $BUILD_USER@${BUILD_HOST} ${BUILD_HOST_PORT} "source py3env/bin/activate && python --version"  2>&1)
echo "virtualenv python version $version"

echo "############ test paramiko import in virtualenv #####"
ssh $BUILD_USER@${BUILD_HOST} ${BUILD_HOST_PORT} "source py3env/bin/activate && python -c \"exec(\\\"try: import paramiko\\\nexcept:\\\\n print( 'missing paramiko')\\\")\""

echo "############ unconditionally clean sandbox folder from VM"
ssh $BUILD_USER@${BUILD_HOST} ${BUILD_HOST_PORT} "if [ ! -f deploy ]; then rm -fr deploy; mkdir deploy; fi"
...
ssh $BUILD_USER@${BUILD_HOST} ${BUILD_HOST_PORT} "source py3env/bin/activate && which pip3 && pip3 install -r deploy/RCM/rcm/client/requirements.txt && python -c \"exec(\\\"try: import paramiko\\\nexcept:\\\\n print( 'missing paramiko')\\\")\""
...
build=$(ssh $BUILD_USER@${BUILD_HOST} ${BUILD_HOST_PORT} "source py3env/bin/activate && cd deploy && pyinstaller RCM/rcm/client/rcm_client_qt.spec ${BUILD_RCM_RELEASE} ${BUILD_PLATFORM}")
